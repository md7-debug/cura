...
- Scroll and Book must contain the same complete normalized source text in the same order. Paginate Book pages from measured font width, line height, usable page height, and paragraph spacing; never let the canvas silently clip words that are absent from the next page. Keep an all-readings bilingual regression test for this invariant.
